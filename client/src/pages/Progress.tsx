...
function ProgressContent() {
    ...
    Promise.all([getProgressSummary(), getLearningHistory()])
      ...
      .catch(() => {
        if (active)
          setError(
            language === "sw"
              ? "Haikuweza kupakia maendeleo yako."
  ...
  const todayPercentage = Math.min(
    (summary.todayProgress / summary.todayGoal) * 100,
    100,
  );
      ...
      <main className="container px-4 py-6 space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-foreground">
            {language === "sw" ? "Maendeleo Yangu" : "My Progress"}
          </h1>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {isLoading ? "-" : summary.totalLearningActivities}
                </p>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {isLoading ? "-" : summary.totalPracticeAttempts}
                </p>
                <p className="text-xs text-muted-foreground">
                  Practice attempts
                </p>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {isLoading ? "-" : `${summary.accuracyPercentage}%`}
                </p>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">
                {summary.todayProgress}/{summary.todayGoal}
              </span>
            </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Practice results</CardTitle>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Topics practiced</CardTitle>
            </CardHeader>
            <CardContent>
              {summary.questionsPracticed.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No practice questions yet.
                </p>
              ) : (
                <ul className="space-y-2">
                  {summary.questionsPracticed.map((question) => (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground capitalize">
                          {item.type === "practice-attempt"
                            ? "Practice attempt"
                            : "Explanation"}
                          {item.subject ? ` · ${item.subject}` : ""}
                          {item.grade ? ` · ${formatGrade(item.grade)}` : ""}
                        </p>
                        {item.selectedAnswer && (
