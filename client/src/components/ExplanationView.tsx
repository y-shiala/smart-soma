...
import { ArrowLeft, Lightbulb, CheckCircle2, HelpCircle, Sparkles, Loader2, BookOpen, Zap } from 'lucide-react';
import { Button } from './ui/button';
import { Card, CardContent, CardHeader, CardTitle } from './ui/card';
import { useLanguage } from '@/contexts/LanguageContext';
import { useState, useEffect } from 'react';
import { generatePracticeQuestion, type PracticeQuestion } from '@/lib/api/ai';
...
interface ExplanationViewProps {
  ...
  onBack: () => void;
  onAskAnother: () => void;
  onSaveProgress: (question: string, subject: string, grade: string) => Promise<void>;
}
...
export function ExplanationView({
  ...
  }, [isStreaming, explanation, explanationMode, progressSaved, onSaveProgress, question, subject, grade]);
  ...
  const parseSteps = (text: string) => {
    ...
    lines.forEach(line => {
      ...
      const isNewStep = /^(\d+[\.\):]|step\s*\d+|hatua\s*\d+)/i.test(line.trim());
  ...
  const allStepsShown = explanationMode === 'direct' || currentStep >= explanationSteps.length - 1;
  const visibleSteps = explanationMode === 'direct' ? explanationSteps : explanationSteps.slice(0, currentStep + 1);

  // Generate practice question when all steps are shown and streaming is done
  useEffect(() => {
    if (!isStreaming && explanation && allStepsShown && !practiceQuestion && !loadingPractice) {
  ...
  }, [isStreaming, explanation, question, subject, language, practiceQuestion, loadingPractice, allStepsShown]);
  ...
  const isCorrect = practiceQuestion ? selectedAnswer === practiceQuestion.correctIndex : false;
    ...
    >
      ...
      <div className="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border/50 px-4 py-3">
        <div className="container flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="font-bold text-foreground leading-tight">{t('explanation')}</h2>
          </div>
        </div>
      </div>
      ...
      <div className="container px-4 py-6 space-y-6">
            ...
            <Card className="transition-shadow duration-300 hover:shadow-md">
              <CardHeader className="pb-3">
                <CardTitle className="text-center text-base">
                  {language === 'sw' ? 'Ungependa kujifunza vipi?' : 'How would you like to learn?'}
                </CardTitle>
              </CardHeader>
              <CardContent className="grid gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {language === 'sw' ? 'Jifunze polepole, hatua moja kwa wakati' : 'Learn slowly, one step at a time'}
                      </p>
                    </div>
                  </div>
                </motion.button>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-foreground">
                        {language === 'sw' ? 'Jibu Moja kwa Moja' : 'Direct Answer'}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {language === 'sw' ? 'Ona jibu fupi na wazi' : 'Get a short, clear answer'}
                      </p>
                    </div>
                  </div>
                </motion.button>
            ...
            <Card className="border-warning/20 transition-shadow duration-300 hover:shadow-md">
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  {explanationMode === 'step-by-step' ? t('stepByStep') : (language === 'sw' ? 'Maelezo' : 'Explanation')}
              ...
              <CardContent className="space-y-4">
                {visibleSteps.length > 0 ? (
                    ...
                    >
                      <Card className="bg-muted/50 border-none shadow-none transition-all duration-200 hover:bg-muted/70">
                        <CardContent className="p-4 flex gap-3">
                          ...
                          <p className="text-foreground leading-relaxed whitespace-pre-line pt-0.5">{step}</p>
                        </CardContent>
                      </Card>
                    </motion.div>
                  ))
                  ...
                  <p className="text-muted-foreground">
                    {isStreaming ? (language === 'sw' ? 'Inafikiri...' : 'Thinking...') : ''}
                  </p>
                )}
                ...
                {explanationMode === 'step-by-step' && !isStreaming && explanationSteps.length > 1 && !allStepsShown && (
                  <motion.div
                ...
                {/* All Steps Complete */}
                {!isStreaming && allStepsShown && explanationSteps.length > 1 && (
                  <motion.div
                  ...
                  >
                    ...
                    {language === 'sw' ? 'Umekamilisha hatua zote!' : 'All steps complete!'}
                  </motion.div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="border-secondary/20 bg-secondary/5 transition-shadow duration-300 hover:shadow-md">
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  {t('tryThis')}
                  {loadingPractice && <Loader2 className="w-4 h-4 animate-spin text-primary" />}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {practiceError && (
                ...
                {practiceQuestion && (
                  <>
                    <p className="text-lg font-semibold text-foreground">{practiceQuestion.question}</p>
                      ...
                      {practiceQuestion.options.map((option, index) => (
                        ...
                        >
                          <span className="inline-flex items-center gap-3">
                            ...
                            {option}
                            {showResult && index === practiceQuestion.correctIndex && (
                    ...
                    {showHint && !showResult && (
                      ...
                      >
                        {language === 'sw' 
                          ? 'Kumbuka hatua tulizojifunza! Fikiria unahitaji kufanya nini.'
                          : 'Remember the steps we just learned! Think about what operation you need.'
                        }
                      </motion.p>
                    )}
                    ...
                    {showResult && (
                      <motion.div
                        ...
                        className={`p-4 rounded-xl ${
                          isCorrect ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning-foreground'
                        }`}
                      >
                        ...
                        {showResult && practiceQuestion.explanation && (
                          <p className="text-sm mt-2 opacity-80">{practiceQuestion.explanation}</p>
                        )}
                      </motion.div>
                    )}
                  </>
                ...
                {!practiceQuestion && !loadingPractice && !practiceError && (
                  <p className="text-muted-foreground text-sm">
                    {language === 'sw' ? 'Inaandaa swali la mazoezi...' : 'Preparing practice question...'}
                  </p>
                )}
              </CardContent>
            </Card>
        ...
        <div className="flex gap-3">
          ...
          <Button variant="default" className="flex-1" onClick={onAskAnother} disabled={isStreaming}>
            {t('askAnother')}
          </Button>
        </div>
      </div>
